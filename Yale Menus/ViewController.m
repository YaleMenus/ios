...
- (NSString*) processTwoMeals : (NSArray*) mealTimes {
    ...
    if (!isToday) {
        ...
        firstMenuItemString = mealTimes.lastObject;
    }
...
- (void)getMealTimes {
    ...
    //will be true if there are any meals at all on the day you're checking, else false
    if (hasBreakfast || hasLunch || hasBrunch || hasDinner) hasAnyMeals = TRUE;
    else hasAnyMeals = FALSE;
...
-(void)mealProcessor : (NSString*)meal withToday : (BOOL)mealTod andTomorrow : (BOOL)mealTom withSwitchpoint : (int)switchpointForMeal {
    ...
    if([meal isEqualToString:@"Contl. Bkfst."]) mealTitle = @"Breakfast", mealTime = mealOneTime;
    if([meal isEqualToString:@"Hot Breakfast"]) mealTitle = @"Hot Breakfast", mealTime = mealOneTime;
    if([meal isEqualToString:@"Brunch"]) mealTitle = @"Brunch", mealTime = mealTwoTime;
    if([meal isEqualToString:@"Lunch"]) mealTitle = @"Lunch", mealTime = mealTwoTime;
    if([meal isEqualToString:@"Dinner"]) mealTitle = @"Dinner", mealTime = mealThreeTime;
    
    //menuMeal.image = [UIImage imageNamed:mealTitle];
    //menuMeal.hidden = FALSE;
    //[self.view addSubview:menuMeal];
    
    //backToMealsButton.hidden = FALSE;
    
    //[self.view addSubview:backToMealsButton];
...
/* ONLY NEEDED WHEN COMING FROM LEGACY MEAL SELECTOR VIEW
 - (void) hideButtonsForMenus {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row % 2 == 0)
        cell.backgroundColor = [UIColor colorWithRed:250/256.0 green:250/256.0 blue:255/256.0 alpha:1.0];
    else cell.backgroundColor = [UIColor colorWithRed:255/256.0 green:255/256.0 blue:255/256.0 alpha:1.0];
    
    //else
    //  cell.textLabel.textColor = [UIColor colorWithRed:255/256.0 green:255/256.0 blue:255/256.0 alpha:1.0];
    cell.textLabel.font = [UIFont fontWithName:@"Optima" size:20.0];
    //cell.backgroundColor =  [UIColor colorWithRed:0.0/256.0 green:37.0/256.0 blue:92.0/256.0 alpha:1.0];
    ...
    if (peanutYesNo == 1 || vegetarianYesNo == 1 || veganYesNo == 1 || dairyYesNo == 1 || glutenYesNo == 1 || eggsYesNo == 1 || soyYesNo == 1 || porkYesNo == 1 || nutsYesNo == 1 || alcoholYesNo == 1 || shellfishYesNo == 1 || seafoodYesNo == 1) {
        ...
        if ([allergens[2] isEqualToString:@"1"] && nutsYesNo == 1) {
        ...
        if ([allergens[3] isEqualToString:@"1"] && shellfishYesNo == 1) {
        ...
        if ([allergens[4] isEqualToString:@"1"] && peanutYesNo == 1) {
        ...
        if ([allergens[5] isEqualToString:@"1"] && dairyYesNo == 1) {
        ...
        if ([allergens[6] isEqualToString:@"1"] && eggsYesNo == 1) {
        ...
        if ([allergens[7] isEqualToString:@"0"] && veganYesNo == 1) { //gray out if vegan trait NOT shown
        ...
        if ([allergens[8] isEqualToString:@"1"] && porkYesNo == 1) {
        ...
        if ([allergens[9] isEqualToString:@"1"] && seafoodYesNo == 1) {
        ...
        if ([allergens[10] isEqualToString:@"1"] && soyYesNo == 1) {
        ...
        if ([allergens[12] isEqualToString:@"1"] && glutenYesNo == 1) {
        ...
        if ([allergens[13] isEqualToString:@"0"] && vegetarianYesNo == 1) { //gray out if vegetarian trait NOT shown
